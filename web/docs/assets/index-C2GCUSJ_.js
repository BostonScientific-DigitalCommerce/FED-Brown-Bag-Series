...
`;a(document.querySelector("#counter"));window.loadMarkdown=async function(n){const t=document.getElementById("content");t.innerHTML="Loading...";try{const r=await fetch(`/content/${n}`);if(!r.ok)throw new Error("Not found");const c=await r.text();t.innerHTML=marked.parse(c)}catch{t.innerHTML="Error loading content."}};
